 ...
 * 例:
 *   毎週日曜 3:51
 *   51 3 * * 0 /path/to/cleanup_search_history.php
 */
...
try {
    do {
        ...
        if ($batchCount % 20 == 0 || $deletedCount == 0) {
            writeLog("バッチ#{$batchCount}: {$deletedCount}件削除完了（{$duration}秒）- 進捗: {$progress}% (" . number_format($deletedTotal) . "/" . number_format($totalToDelete) . ")");
        }

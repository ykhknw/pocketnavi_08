...
class SingleRecordMigrator {
    ...
    public function runTest() {
        echo "<h2>検索履歴filters移行テスト（1件のみ）</h2>";
        echo "<hr>";
        ...
        // 3. building_idの確認
        $buildingId = $currentFilters['building_id'] ?? null;
        if (!$buildingId) {
            echo "<p style='color: red;'>✗ building_idが見つかりません</p>";
            return;
        }
        
        echo "<p><strong>Building ID:</strong> {$buildingId}</p>";
        
        // 4. 建築物データの取得
        $buildingData = $this->getBuildingData($buildingId);
        if (!$buildingData) {
            echo "<p style='color: red;'>✗ 建築物データが見つかりません (building_id: {$buildingId})</p>";
            return;
        }
        ...
        echo "<table border='1' cellpadding='5' cellspacing='0'>";
        echo "<tr><th>項目</th><th>値</th></tr>";
        echo "<tr><td>Building ID</td><td>{$buildingData['building_id']}</td></tr>";
        echo "<tr><td>Slug</td><td>" . htmlspecialchars($buildingData['slug']) . "</td></tr>";
        echo "<tr><td>Title (JA)</td><td>" . htmlspecialchars($buildingData['title']) . "</td></tr>";
        echo "<tr><td>Title (EN)</td><td>" . htmlspecialchars($buildingData['titleEn']) . "</td></tr>";
        echo "</table>";
        ...
        if (isset($_GET['execute']) && $_GET['execute'] === 'yes') {
            ...
            if ($result) {
                ...
                echo "<p style='color: green; font-weight: bold;'>✓ 移行テスト完了！</p>";
            } else {
    ...
    private function getBuildingData($buildingId) {
        $sql = "SELECT building_id, slug, title, titleEn FROM buildings_table_3 WHERE building_id = ?";
        $stmt = $this->db->prepare($sql);
...
// 実行部分
try {
    $migrator = new SingleRecordMigrator();
    $migrator->runTest();
} catch (Exception $e) {
